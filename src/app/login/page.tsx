      ...
      <div className="relative mx-auto grid min-h-[calc(100dvh-24px)] max-w-[1380px] overflow-hidden rounded-[30px] border border-black/[0.06] bg-white shadow-[0_30px_100px_rgba(28,38,58,.13)] dark:border-white/[0.07] dark:bg-[#11141a] dark:shadow-[0_30px_100px_rgba(0,0,0,.4)] sm:min-h-[calc(100dvh-40px)] lg:grid-cols-[1.05fr_.95fr] lg:min-h-[calc(100dvh-48px)]">
        ...
        <section className="flex items-center justify-center px-5 py-10 sm:px-10 lg:px-12 xl:px-20">
          <div className="w-full max-w-[460px]">
            ...
            <div className="mb-8"><p className="page-eyebrow mb-3">Secure workspace</p><h2 className="text-[2.2rem] font-black leading-none tracking-[-0.05em] sm:text-[2.6rem]">Welcome back.</h2><p className="mt-3 text-sm leading-6 text-[var(--muted)]">Sign in with your NexiMail account.</p></div>
            {params.error ? <div role="alert" className="mb-5 rounded-2xl border border-rose-500/15 bg-rose-500/[0.07] px-4 py-3.5 text-sm font-semibold text-rose-700 dark:text-rose-300">{getLoginErrorMessage(params.error)}</div> : null}
            <form method="post" action="/api/auth/login" className="space-y-4">
              <label className="block"><span className="mb-2 block text-[12px] font-extrabold">Email address</span><div className="flex h-[52px] items-center rounded-[15px] border border-[var(--border-strong)] bg-[var(--surface-soft)] px-3.5 transition focus-within:border-violet-500/50 focus-within:bg-[var(--surface)] focus-within:ring-4 focus-within:ring-violet-500/[0.07]"><Mail className="h-[17px] w-[17px] text-[var(--muted)]"/><input name="email" type="email" autoComplete="email" required placeholder="owner@example.com" className="h-full w-full bg-transparent px-3 text-sm font-medium outline-none placeholder:font-normal placeholder:text-[var(--muted)]"/></div></label>
              <label className="block"><span className="mb-2 block text-[12px] font-extrabold">Password</span><div className="flex h-[52px] items-center rounded-[15px] border border-[var(--border-strong)] bg-[var(--surface-soft)] px-3.5 transition focus-within:border-violet-500/50 focus-within:bg-[var(--surface)] focus-within:ring-4 focus-within:ring-violet-500/[0.07]"><LockKeyhole className="h-[17px] w-[17px] text-[var(--muted)]"/><input name="password" type="password" autoComplete="current-password" required placeholder="••••••••••••" className="h-full w-full bg-transparent px-3 text-sm font-medium outline-none placeholder:font-normal placeholder:text-[var(--muted)]"/></div></label>
              <button type="submit" className="btn-primary mt-2 h-[50px] w-full !rounded-[15px]">Sign in <ArrowRight className="h-4 w-4"/></button>
            </form>
          </div>
        </section>
      </div>
