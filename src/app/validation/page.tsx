  ...
  if (databaseConfigured) {
    try {
      const [jobRows, gmailRows, acceptedRows, invalidRows, unknownRows, pendingRows] = await Promise.all([
        ...
        db.select({value:sql<number>`count(*)::int`}).from(contacts).where(sql`lower(${contacts.normalizedEmail}) ~ '@(gmail|googlemail)\\.com$'`),
        db.select({value:sql<number>`count(*)::int`}).from(contacts).where(sql`${contacts.validationStatus}='accepted'`),
        db.select({value:sql<number>`count(*)::int`}).from(contacts).where(sql`${contacts.validationStatus}='invalid'`),
        db.select({value:sql<number>`count(*)::int`}).from(contacts).where(sql`${contacts.validationStatus} in ('unknown','error')`),
        db.select({value:sql<number>`count(*)::int`}).from(contacts).where(sql`${contacts.validationStatus}='pending'`),
      ]);
