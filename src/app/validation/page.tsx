...
export default async function ValidationPage() {
  ...
  return <AppShell session={session}>
    ...
    <section className="mb-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
      ...
      ].map((x)=><article key={x.l} className="compact-stat"><div className="flex items-end justify-between gap-3"><div><p className="compact-stat-label">{x.l}</p><p className="compact-stat-value">{usable ? x.v.toLocaleString() : "—"}</p></div><p className="text-right text-[11px] font-semibold text-[var(--muted)]">{x.n}</p></div></article>)}
    </section>
