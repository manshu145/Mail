  ...
  return <AppShell session={session}>
    ...
    <section className="grid gap-5 xl:grid-cols-[.75fr_1.25fr]">
      <div className="space-y-5">
        <article className="premium-panel p-6"><h2 className="font-black">Delivery details</h2><div className="mt-5 space-y-3 text-sm">{[["Campaign",row.campaignName],["Subject",row.campaignSubject],["Mailbox provider",providerLabel(provider)],["Validation",row.contactValidation],["Provider queue ID",m.providerMessageId||"—"],["Queued",fmt(m.queuedAt)],["Accepted",fmt(m.acceptedAt)],["Delivered",fmt(m.deliveredAt)],["Bounced",fmt(m.bouncedAt)]].map(([a,b])=><div key={String(a)} className="flex gap-4 border-b border-[var(--border)] pb-3 last:border-0"><span className="w-36 shrink-0 text-[var(--muted)]">{a}</span><b className="break-all">{String(b)}</b></div>)}</div>{m.lastError?<div className="mt-5 rounded-xl bg-rose-500/10 p-4 text-xs leading-5 text-rose-700 dark:text-rose-300"><b>Last delivery response</b><div className="mt-1 break-words">{m.lastError}</div></div>:null}</article>
      ...
      <article className="premium-panel overflow-hidden"><div className="border-b border-[var(--border)] p-5"><h2 className="font-black">Full event timeline</h2><p className="mt-1 text-xs text-[var(--muted)]">Transport attempts, provider responses, opens, clicks, unsubscribe, complaint and bounce events for this person/message.</p></div>{events.length?<div className="divide-y divide-[var(--border)]">{events.map((e)=>{const payload=e.payload||{};const automated=Boolean(payload.automated);const url=typeof payload.url==="string"?payload.url:null;return <div key={e.id} className="p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><span className="rounded-full bg-violet-500/10 px-2.5 py-1 text-[11px] font-black uppercase tracking-wide text-violet-700 dark:text-violet-300">{e.type.replaceAll("_"," ")}</span>{automated?<span className="text-[11px] font-bold text-amber-600">automated</span>:null}</div>{url?<a className="mt-2 block break-all text-sm font-bold text-violet-600 hover:underline" href={url} target="_blank" rel="noreferrer">{url}</a>:null}<pre className="mt-2 whitespace-pre-wrap break-words font-sans text-xs leading-5 text-[var(--muted)]">{Object.keys(payload).length?JSON.stringify(payload,null,2):"No additional payload"}</pre></div><time className="shrink-0 text-xs text-[var(--muted)]">{fmt(e.createdAt)}</time></div></div>})}</div>:<div className="grid min-h-48 place-items-center p-8 text-center text-sm text-[var(--muted)]"><Clock3 className="mb-3 h-7 w-7"/>No events recorded yet.</div>}</article>
    </section>
  </AppShell>;
}
