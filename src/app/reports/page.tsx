...
export default async function ReportsPage(){
  ...
  return <AppShell session={session}>
    ...
    <section className="premium-panel mt-5 overflow-hidden"><div className="border-b border-[var(--border)] p-5"><h2 className="font-black">Provider performance & cooldowns</h2><p className="mt-1 text-xs text-[var(--muted)]">Provider cooldowns are reserved for temporary provider pressure such as throttling/rate-limit responses; single permanent content/policy rejections do not pause a provider.</p></div><div className="grid gap-5 p-5 xl:grid-cols-[1.4fr_.6fr]"><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-[10px] font-black uppercase tracking-[.12em] text-[var(--muted)]"><tr><th>Provider</th><th>Total</th><th>Remote accepted</th><th>Deferred</th><th>Bounced</th><th>Failed</th></tr></thead><tbody>{providers.length?providers.map(([p,x])=><tr key={p} className="border-t border-[var(--border)]"><td className="py-3 font-bold">{providerLabel(p)}</td><td>{x.total}</td><td>{x.delivered}</td><td>{x.deferred}</td><td>{x.bounced}</td><td>{x.failed}</td></tr>):<tr><td colSpan={6} className="py-8 text-center text-[var(--muted)]">No provider data yet.</td></tr>}</tbody></table></div><div><h3 className="text-sm font-black">Active cooldowns</h3><div className="mt-3 space-y-2">{activeCooldowns.length?activeCooldowns.map(c=><div key={c.id} className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-200"><b>{providerLabel(c.provider)}</b><div className="mt-1">Next probe: {c.nextProbeAt?new Intl.DateTimeFormat("en",{dateStyle:"short",timeStyle:"short", timeZone:"Asia/Kolkata"}).format(c.nextProbeAt):"due now"}</div><div className="mt-1 break-words opacity-80">{c.reason||c.lastResponse||"Provider pressure detected"}</div></div>):<div className="rounded-xl bg-emerald-500/10 p-4 text-xs font-bold text-emerald-700 dark:text-emerald-300">No active provider cooldowns.</div>}</div></div></div></section>
