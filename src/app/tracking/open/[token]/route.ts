...
export async function GET(request: Request, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    if (databaseConfigured && messageId && /^[0-9a-f-]{36}$/i.test(messageId)) {
      ...
      await db.insert(messageEvents).values({ messageId, type: "open", payload: { source: "tracking_pixel", ...classification } });
      if (!classification.automated) await emitWebhookEvent("message.opened", { messageId }).catch((error) => console.error("[tracking.open.webhook]", error));
    }
