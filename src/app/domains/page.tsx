          ...
          <div className="p-4 sm:p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2"><div className="grid h-9 w-9 place-items-center rounded-xl bg-violet-500/10 text-[var(--accent)]"><Globe2 className="h-4.5 w-4.5"/></div><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-black">{r.domain}</h2><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-[.08em] ${r.status==="ready"?"bg-emerald-500/10 text-emerald-700 dark:text-emerald-300":r.status==="disabled"?"bg-[var(--surface-muted)] text-[var(--muted)]":"bg-amber-500/10 text-amber-700 dark:text-amber-300"}`}>{r.status}</span></div><p className="mt-1 text-xs text-[var(--muted)]">Tracking: {r.trackingDomain||"default application domain"}</p></div></div>
              </div>
