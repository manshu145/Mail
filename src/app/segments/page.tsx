...
export default async function SegmentsPage() {
  ...
  return <AppShell session={session}>
    ...
    <section className="mb-4 grid gap-2 sm:grid-cols-4">{[{label:"Rule segments",value:segments.length,icon:Blocks},{label:"Engagement segments",value:engagementSegments.length,icon:MousePointerClick},{label:"Lists",value:listRows.length,icon:Layers3},{label:"Tag assignments",value:totalAssignments,icon:UsersRound}].map(({label,value,icon:Icon}) => <article key={label} className="compact-stat"><div className="flex items-start justify-between"><div><p className="compact-stat-label">{label}</p><p className="compact-stat-value">{value.toLocaleString()}</p></div><div className="grid h-8 w-8 place-items-center rounded-lg bg-violet-500/[0.08] text-violet-700 dark:text-violet-300"><Icon className="h-4.5 w-4.5" /></div></div></article>)}</section>

    <section className="premium-panel mb-4 overflow-hidden"><div className="border-b border-[var(--border)] px-4 py-3.5"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[var(--muted)]">Campaign behavior</p><h2 className="mt-1 text-lg font-black">Engagement audiences</h2><p className="mt-1 text-xs text-[var(--muted)]">Automated opens/clicks are excluded. Dynamic audiences resolve again when a campaign starts.</p></div>{engagementSegments.length ? <div className="divide-y divide-[var(--border)]">{engagementSegments.map(row => <div key={row.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div><p className="text-sm font-extrabold">{row.name}</p><p className="mt-1 text-xs text-[var(--muted)]">{row.description || row.campaignName}</p></div><div className="text-right"><p className="text-xs font-black">{row.ruleType.replaceAll("_", " ")}</p><p className="mt-1 text-[11px] text-[var(--muted)]">{row.campaignName}{row.windowDays ? ` · last ${row.windowDays}d` : " · full campaign"}</p></div></div>)}</div> : <div className="p-8 text-center text-sm text-[var(--muted)]">No engagement audiences yet.</div>}</section>

    <section className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
      <article className="premium-panel overflow-hidden"><div className="border-b border-[var(--border)] px-5 py-4 sm:px-6"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[var(--muted)]">Dynamic targeting</p><h2 className="mt-1 text-lg font-black">Contact-rule segments</h2></div>{segments.length ? <div className="divide-y divide-[var(--border)]">{segments.map((row) => <div key={row.id} className="px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-extrabold">{row.name}</p><p className="mt-1 text-xs text-[var(--muted)]">{row.description || "Dynamic audience"}</p></div><code className="rounded-lg border border-[var(--border)] bg-[var(--surface-soft)] px-2.5 py-1.5 text-[11px] font-bold">{row.field === "custom_attribute" ? `custom.${row.attributeKey || "?"}` : row.field} {row.operator.replace("_", " ")} {row.value}</code></div></div>)}</div> : <div className="p-8 text-center text-sm text-[var(--muted)]">No contact-rule segments yet.</div>}</article>
      <article className="premium-panel overflow-hidden"><div className="border-b border-[var(--border)] px-5 py-4 sm:px-6"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[var(--muted)]">Targeting taxonomy</p><h2 className="mt-1 text-lg font-black">Tags</h2></div>{tagRows.length ? <div className="p-4"><div className="flex flex-wrap gap-2">{tagRows.map((row) => <span key={row.id} className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface-soft)] px-3 py-2 text-xs font-bold"><span>{row.name}</span><span className="rounded-full bg-violet-500/10 px-1.5 py-0.5 text-[10px] font-black text-violet-700 dark:text-violet-300">{row.members}</span></span>)}</div></div> : <div className="p-8 text-center text-sm text-[var(--muted)]">No tags yet.</div>}</article>
    </section>
  </AppShell>;
}
