...
export async function POST(request:NextRequest,{params}:{params:Promise<{resource:string}>}){
 ...
 try{
  ...
  if(resource==="settings"){if(session.role==="operator")return NextResponse.json({error:"Admin or owner role required."},{status:403});const key=text(body.key);if(!key)return NextResponse.json({error:"Setting key is required."},{status:400});const value=body.value??null;await db.insert(systemSettings).values({key,value}).onConflictDoUpdate({target:systemSettings.key,set:{value,updatedAt:new Date()}});await audit("setting.updated",session,"setting",key);return NextResponse.json({ok:true})}
  if(resource==="domains"){
  ...
  if(resource==="inbox-tests"){const name=text(body.name);if(!name)return NextResponse.json({error:"Test name is required."},{status:400});const campaignId=text(body.campaignId)||null;const rows=await db.insert(inboxTests).values({name,campaignId,status:"draft"}).returning({id:inboxTests.id});await audit("inbox_test.created",session,"inbox_test",rows[0].id,{name,campaignId});return NextResponse.json({ok:true,id:rows[0].id},{status:201})}
