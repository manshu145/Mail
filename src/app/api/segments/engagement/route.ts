...
export async function POST(request: NextRequest) {
  ...
  const ruleType = String(body?.ruleType || "") as EngagementRule;
  ...
  if (!name || !campaignId || !ENGAGEMENT_RULES.includes(ruleType)) return NextResponse.json({ error: "Name, campaign and engagement rule are required." }, { status: 400 });
