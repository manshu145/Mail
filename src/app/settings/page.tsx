  ...
  return (
    <AppShell session={session}>
      <div className="mb-7 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="page-eyebrow mb-2">Workspace</p><h1 className="page-title">Settings</h1><p className="page-description">Manage safe workspace-level defaults while infrastructure secrets remain isolated in the VPS environment.</p></div>
      ...
      {!usable ? <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/25 dark:text-amber-200"><b>Settings storage unavailable.</b> Preview mutations remain disabled until PostgreSQL is connected.</div> : null}
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
        <article className="premium-panel overflow-hidden">
          ...
          {rows.length ? <div className="divide-y divide-[var(--border)]">{rows.map((row) => <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-6" key={row.key}><div className="min-w-0"><div className="font-mono text-sm font-bold">{row.key}</div><div className="mt-1 max-w-xl truncate text-xs text-[var(--muted)]">{JSON.stringify(row.value)}</div></div><span className="shrink-0 text-xs font-bold text-[var(--muted)]">{new Intl.DateTimeFormat("en",{dateStyle:"medium"}).format(row.updatedAt)}</span></div>)}</div> : <div className="grid min-h-64 place-items-center p-8 text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[var(--surface-soft)] text-[var(--muted)]"><Settings2 className="h-5 w-5" /></div><h2 className="mt-4 font-black">No workspace overrides</h2><p className="mt-1 text-sm text-[var(--muted)]">Defaults stay in code until an owner persists an override.</p></div></div>}
        </article>

        <aside className="space-y-4">
          <article className="premium-panel p-5 sm:p-6"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"><ShieldCheck className="h-5 w-5" /></div><div><h2 className="font-black">Security boundary</h2><p className="mt-1 text-sm leading-6 text-[var(--muted)]">SMTP passwords, database URLs, Redis URLs, DKIM private keys and signing secrets never belong in this settings table.</p></div></div></article>
          <article className="premium-panel p-5 sm:p-6"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-violet-500/10 text-violet-700 dark:text-violet-300"><SlidersHorizontal className="h-5 w-5" /></div><div><h2 className="font-black">Runtime controls</h2><p className="mt-1 text-sm leading-6 text-[var(--muted)]">Sending limits and reputation controls stay in their dedicated operational modules instead of being mixed with generic workspace preferences.</p></div></div></article>
