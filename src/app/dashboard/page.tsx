  ...
  return <AppShell session={session}>
    ...
    <section className="mt-5 grid gap-5 xl:grid-cols-2">
      ...
      <article className="premium-panel overflow-hidden">
        ...
        {activeSubscribers.length?<div className="divide-y divide-[var(--border)]">{activeSubscribers.map((row)=><a key={String(row.id)} href={`/contacts/${String(row.id)}`} className="flex items-center justify-between gap-4 px-4 py-3 transition hover:bg-[var(--surface-soft)]"><div className="min-w-0"><p className="truncate text-[12px] font-extrabold">{String(row.name)}</p><p className="mt-1 truncate text-[11px] text-[var(--muted)]">{String(row.email)}</p></div><div className="flex shrink-0 gap-4 text-right"><div><p className="text-[10px] font-bold text-[var(--muted)]">Opens</p><p className="text-[12px] font-black">{Number(row.opens||0).toLocaleString()}</p></div><div><p className="text-[9px] font-bold text-[var(--muted)]">Clicks</p><p className="text-[12px] font-black">{Number(row.clicks||0).toLocaleString()}</p></div></div></a>)}</div>:<div className="p-6 text-center text-xs text-[var(--muted)]">No engagement activity yet.</div>}
      </article>
    </section>

    <section className="premium-panel mt-5 p-5 xl:p-6">
      <div className="mb-4"><p className="page-eyebrow">Quick access</p><h2 className="mt-1 text-base font-black">Manage your workspace</h2></div>
