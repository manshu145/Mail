...
export default async function DashboardPage() {
  ...
  return <AppShell session={session}>
    ...
    <section className="premium-panel mt-4 p-4">
      <div className="mb-3"><p className="page-eyebrow">Quick access</p><h2 className="mt-1 text-base font-black">Manage your workspace</h2></div>
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {[[UsersRound,"Contacts","Manage audience","/contacts"],[Mail,"Templates","Create email content","/templates"],[Globe2,"Sending domains","Manage verified domains","/domains"],[MailCheck,"Reports","View campaign results","/reports"]].map(([Icon,label,note,href])=>{const Component=Icon as typeof UsersRound;return <a key={String(label)} href={String(href)} className="panel-soft flex items-center gap-3 p-3 transition hover:border-violet-500/30 hover:bg-violet-500/[0.04]"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-violet-500/[0.08] text-violet-700 dark:text-violet-300"><Component className="h-4 w-4"/></div><div className="min-w-0"><p className="text-[11px] font-extrabold">{String(label)}</p><p className="text-[9.5px] text-[var(--muted)]">{String(note)}</p></div></a>})}
      </div>
    </section>
  </AppShell>;
}
