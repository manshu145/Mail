  ...
  return <AppShell session={session}>
    ...
    <section className="mt-4 grid gap-4 xl:grid-cols-2">
      <article className="premium-panel overflow-hidden">
        ...
        {recentCampaigns.length?<div className="divide-y divide-[var(--border)]">{recentCampaigns.map((row)=><a key={String(row.id)} href={`/campaigns/${String(row.id)}`} className="flex items-center justify-between gap-4 px-4 py-3 transition hover:bg-[var(--surface-soft)]"><div className="min-w-0"><p className="truncate text-[12px] font-extrabold">{String(row.name)}</p><p className="mt-0.5 text-[10px] text-[var(--muted)]">{new Intl.DateTimeFormat("en",{dateStyle:"medium",timeStyle:"short"}).format(new Date(String(row.created_at)))}</p></div><div className="flex shrink-0 items-center gap-4 text-right"><div><p className="text-[10px] font-bold text-[var(--muted)]">Delivered</p><p className="text-[12px] font-black">{Number(row.delivered||0).toLocaleString()} / {Number(row.targeted||0).toLocaleString()}</p></div><span className="rounded-full bg-violet-500/10 px-2 py-1 text-[9px] font-black capitalize text-violet-700 dark:text-violet-300">{String(row.status)}</span></div></a>)}</div>:<div className="p-6 text-center text-xs text-[var(--muted)]">No campaigns yet.</div>}
      </article>
