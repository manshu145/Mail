...
export default async function SystemHealthPage() {
  ...
  const items = [
    ...
    ...workerNames.map((name) => ({ name: `${name.replaceAll("-", " ")} worker`, status: workerState(name), detail: beat.get(name) ? `Last heartbeat ${new Intl.DateTimeFormat("en",{dateStyle: "medium", timeStyle: "short", timeZone:"Asia/Kolkata"}).format(beat.get(name)!.lastSeenAt)}` : "No heartbeat recorded", icon: ServerCog })),
  ];
