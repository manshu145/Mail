...
export default async function SendingLimitsPage() {
  ...
  return <AppShell session={session}>
    <div className="mb-5"><p className="page-eyebrow mb-2">Delivery controls</p><h1 className="page-title">Sending limits</h1><p className="page-description">Configure workspace sending limits, retry behavior and reputation safeguards from one place.</p></div>
    {!currentSettings ? <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/25 dark:text-amber-200">Sending-limit settings are temporarily unavailable.</div> : null}
    {currentSettings ? <>
      ...
      ].map(([l, v]) => <article className="compact-stat" key={String(l)}><p className="compact-stat-label">{l}</p><p className="compact-stat-value">{v}</p></article>)}</section>

      <section className="premium-panel p-4"><div className="mb-5 flex items-start gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-violet-500/10 text-violet-700 dark:text-violet-300"><Gauge className="h-5 w-5"/></div><div><h2 className="font-black">Throughput, retry & reputation guard</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">Owner changes are audited and applied consistently across NexiMail delivery operations.</p></div></div><DeliverySettingsForm initial={currentSettings} editable={session.role === "owner"}/></section>

      <section className="mt-4 grid gap-3 lg:grid-cols-2"><article className="premium-panel p-5"><div className="flex gap-3"><TimerReset className="mt-0.5 h-5 w-5 text-amber-500"/><div><h3 className="font-black">Retry policy</h3><p className="mt-2 text-sm leading-6 text-[var(--muted)]">Up to {currentSettings.retryMaxAttempts} delivery attempts with an initial {currentSettings.retryInitialSeconds}s delay, progressive ×{currentSettings.retryBackoffMultiplier} backoff and a {currentSettings.retryMaxSeconds}s maximum wait.</p></div></div></article><article className="premium-panel p-5"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-emerald-600"/><div><h3 className="font-black">Reputation guard</h3><p className="mt-2 text-sm leading-6 text-[var(--muted)]">After at least {currentSettings.reputationMinSample.toLocaleString()} 24-hour sends, an account can be paused at {(currentSettings.reputationBounceStopRate * 100).toFixed(2)}% bounce or {(currentSettings.reputationComplaintStopRate * 100).toFixed(3)}% complaint rate.</p></div></div></article></section>
    </> : null}
  </AppShell>;
}
