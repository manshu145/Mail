...
export default async function SendingLimitsPage() {
  ...
  return <AppShell session={session}>
    <div className="mb-5"><p className="page-eyebrow mb-2">Delivery controls</p><h1 className="page-title">Sending limits</h1><p className="page-description">Every editable value on this page is stored in the database control plane and is intended for worker enforcement rather than decorative configuration.</p></div>
    ...
    {currentSettings ? <>
      ...
      <section className="premium-panel p-4"><div className="mb-5 flex items-start gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-violet-500/10 text-violet-700 dark:text-violet-300"><Gauge className="h-5 w-5"/></div><div><h2 className="font-black">Throughput, retry & reputation guard</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">Owner changes are audited. Transport, policy, campaign and reputation workers consume the shared settings instead of relying on UI-only values.</p></div></div><DeliverySettingsForm initial={currentSettings} editable={session.role === "owner"}/></section>
