...
export default async function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell session={session}>
    ...
    {recipientRows.length ? <section className="premium-panel mt-5 overflow-hidden"><div className="border-b border-[var(--border)] p-5"><h2 className="font-black">Recipient delivery log</h2><p className="mt-1 text-xs text-[var(--muted)]">Latest 100 recipient records with verified engagement.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="bg-[var(--surface-soft)] text-[10px] font-black uppercase tracking-[.12em] text-[var(--muted)]"><tr><th className="px-5 py-3">Recipient</th><th>Status</th><th>Opens</th><th>Clicks</th><th>Accepted</th><th>Delivered</th><th>Last activity</th><th>Error</th></tr></thead><tbody>{recipientRows.map((m)=><tr className="border-t border-[var(--border)]" key={String(m.id)}><td className="px-5 py-3.5 font-bold">{String(m.recipient_email)}</td><td className="capitalize">{String(m.status).replaceAll("_"," ")}</td><td>{Number(m.opens||0).toLocaleString()}</td><td>{Number(m.clicks||0).toLocaleString()}</td><td className="text-xs text-[var(--muted)]">{fmt(m.accepted_at as string|null)}</td><td className="text-xs text-[var(--muted)]">{fmt(m.delivered_at as string|null)}</td><td className="text-xs text-[var(--muted)]">{fmt(m.last_activity as string|null)}</td><td className="max-w-[260px] truncate text-xs text-rose-500">{String(m.last_error||"—")}</td></tr>)}</tbody></table></div></section> : null}
  </AppShell>;
}
