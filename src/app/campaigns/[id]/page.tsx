  ...
  return <AppShell session={session}>
    <div className="mb-7">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2"><span className="rounded-full bg-violet-500/10 px-3 py-1.5 text-xs font-extrabold capitalize text-violet-700 dark:text-violet-300">{campaign.status}</span><DuplicateCampaignButton id={campaign.id}/><CampaignControl id={campaign.id} status={campaign.status} failed={metrics?.failed||0}/></div>
      </div>
    </div>
    ...
    {preflight?<section className="premium-panel mb-5 overflow-hidden"><div className="border-b border-[var(--border)] p-5"><h2 className="font-black">Audience preflight</h2><p className="mt-1 text-xs text-[var(--muted)]">Eligibility snapshot before delivery. Suppressed contacts and contacts already marked invalid are excluded. Pending, unknown or not-yet-validated contacts remain eligible; validation is a separate optional workflow.</p></div><div className="grid gap-px bg-[var(--border)] sm:grid-cols-2 xl:grid-cols-7">{[["Active opt-ins",preflight.rawCount],["Eligible",preflight.eligibleCount],["Suppressed",preflight.suppressedCount],["Invalid",preflight.invalidCount],["Valid",preflight.validCount],["Pending / unvalidated",preflight.pendingCount],["Unknown",preflight.unknownCount]].map(([label,value])=><div className="bg-[var(--surface)] p-4" key={String(label)}><p className="text-xs font-bold text-[var(--muted)]">{label}</p><p className="mt-1 text-2xl font-black">{Number(value).toLocaleString()}</p></div>)}</div><div className="border-t border-[var(--border)] px-5 py-3 text-xs text-[var(--muted)]">Last checked {fmt(preflight.checkedAt)}</div></section>:null}
