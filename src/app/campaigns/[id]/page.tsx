  ...
  return <AppShell session={session}>
    <div className="mb-7">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="page-eyebrow">Campaign builder</p><h1 className="mt-2 text-3xl font-black tracking-[-.035em] sm:text-4xl">{campaign.name}</h1><p className="mt-2 text-xs text-[var(--muted)]">Already accepted mail cannot be recalled; cancellation only stops recipient messages still controlled by NexiMail.</p></div>
    ...
    <section className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Queued", state.queued || 0], ["Ready", state.ready_for_transport || 0], ["MTA accepted", state.mta_accepted || 0], ["Delivered", state.delivered || 0]].map(([label, count]) => <article className="premium-panel p-5" key={String(label)}><p className="text-sm font-bold text-[var(--muted)]">{label}</p><p className="mt-2 text-3xl font-black">{Number(count).toLocaleString()}</p></article>)}</section>
    <section className="premium-panel p-6">{editable ? <CampaignEditor campaign={{ ...campaign, scheduledAt: campaign.scheduledAt?.toISOString() || null }} lists={listRows} templates={templateRows} accounts={accountRows} runtimePolicy={{ mode: runtimePolicy.mode, sendingEnabled: runtimePolicy.sendingEnabled, maxRecipientsPerCampaign: runtimePolicy.maxRecipientsPerCampaign }} /> : <div className="grid min-h-52 place-items-center text-center"><div><CircleGauge className="mx-auto h-8 w-8 text-[var(--muted)]" /><h2 className="mt-4 font-black">Campaign is in the sending pipeline</h2><p className="mt-1 text-sm text-[var(--muted)]">Editing is locked once worker processing begins.</p></div></div>}</section>
  </AppShell>;
}
