  ...
  return <AppShell session={session}>
    <div className="mb-7">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2"><span className="rounded-full bg-violet-500/10 px-3 py-1.5 text-xs font-extrabold capitalize text-violet-700 dark:text-violet-300">{campaign.status}</span><CampaignReuseAction campaignId={campaign.id} lists={listRows} currentListId={campaign.listId}/><CampaignControl id={campaign.id} status={campaign.status} failed={metrics?.failed||0}/></div>
      </div>
    </div>
