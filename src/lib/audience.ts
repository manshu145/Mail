...
export async function resolveAudienceRecipients(list: typeof lists.$inferSelect): Promise<AudienceRecipient[]> {
  ...
  const [rule] = await db
    ...
    .where(eq(segmentDefinitions.listId, list.id))
    .limit(1);
  ...
  if (rule.field === "email_domain") {
    ...
    const status = rule.value as "pending" | "valid" | "invalid" | "unknown" | "error";
    condition = rule.operator === "equals"
      ? eq(contacts.validationStatus, status)
      : ne(contacts.validationStatus, status);
  } else {
    const status = rule.value as "active" | "archived";
    condition = rule.operator === "equals"
      ? eq(contacts.status, status)
      : ne(contacts.status, status);
  }
