...
/**
 * Validation audience relation.
 *
 * Validation is mailbox-state discovery, not a send-eligibility check:
 * do not require confirmed consent/source or recipient-domain health here.
 * Those safeguards remain part of campaign sending.
 */
export async function validationAudienceSelection(list: typeof lists.$inferSelect, executor: Pick<typeof db, "select"> = db): Promise<SQL> {
  let condition: SQL | undefined;
  if (!list.isDynamic) {
    condition = sql`exists(select 1 from contact_lists cl where cl.contact_id=${contacts.id} and cl.list_id=${list.id})`;
  } else {
