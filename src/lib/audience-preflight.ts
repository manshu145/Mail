...
export async function preflightAudience(list: typeof lists.$inferSelect): Promise<AudiencePreflightResult> {
  ...
  const result = await db.execute(sql`with audience as (${refreshedSelection}) select
    ...
    count(*) filter(where not suppressed and not domain_eligible)::int as domain_invalid_count,
  ...
  return { rawCount: Number(row.raw_count), eligibleCount: Number(row.eligible_count), suppressedCount: Number(row.suppressed_count),
    ...
    domainInvalidCount: Number(row.domain_invalid_count), domainHealthPendingCount: domainHealth.pendingDomains, checkedDomainCount: domainHealth.checkedDomains };
}
