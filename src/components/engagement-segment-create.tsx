...
export function EngagementSegmentCreate({ campaigns, disabled }: { campaigns: CampaignOption[]; disabled?: boolean }) {
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-[90] grid place-items-center overflow-y-auto bg-black/50 p-4 backdrop-blur-sm"><section className="premium-panel max-h-[calc(100dvh-2rem)] w-full max-w-xl overflow-y-auto p-5 sm:p-6"><div className="mb-5 flex items-start justify-between"><div><p className="page-eyebrow">Engagement segmentation</p><h2 className="mt-1 text-2xl font-black">Build from campaign activity</h2><p className="mt-1 text-xs text-[var(--muted)]">Create a live segment or freeze the current matching audience into a static list.</p></div><button className="icon-button grid" onClick={() => { setOpen(false); setPreviewCount(null); }}><X className="h-4 w-4"/></button></div><form action={create} className="space-y-4" ref={(form) => { if (form) (form as HTMLFormElement & { previewAudience?: () => void }).previewAudience = () => preview(form); }}>
