...
export function CampaignEditor({ campaign, lists, templates, accounts, runtimePolicy }: { campaign: Campaign; lists: Option[]; templates: Option[]; accounts: AccountOption[]; runtimePolicy: RuntimePolicyView }) {
  ...
  return <form className="space-y-5" action={async (formData) => submit(formData, "save")}>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-5">
      <div className="flex flex-wrap gap-2"><button disabled={busy} className="btn-secondary" type="submit">Save draft</button><button disabled={busy || !runtimePolicy.sendingEnabled} className="btn-secondary" type="button" onClick={()=>setShowTest(true)}>Send test</button></div>
      <div className="flex flex-wrap gap-2"><button disabled={busy || !runtimePolicy.sendingEnabled || !schedule} className="btn-secondary" type="button" onClick={(event)=>{const form=event.currentTarget.form;if(form)submit(new FormData(form),"schedule")}}>Schedule</button><button disabled={busy || !runtimePolicy.sendingEnabled} className="btn-primary" type="button" onClick={(event)=>{const form=event.currentTarget.form;if(form)submit(new FormData(form),"send_now")}}>{busy ? "Working…" : "Send now"}</button></div>
    </div>
  </form>;
}
