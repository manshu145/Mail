...
export function CampaignEditor({ campaign, lists, templates, accounts, runtimePolicy }: { campaign: Campaign; lists: Option[]; templates: Option[]; accounts: Option[]; runtimePolicy: RuntimePolicyView }) {
  ...
  const staging = runtimePolicy.mode === "staging";

  return <form className="space-y-5" action={async (formData) => submit(formData, "save")}>
    {staging ? <div className="flex flex-col gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/[0.07] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-amber-500/10 text-amber-700 dark:text-amber-300"><ShieldCheck className="h-4.5 w-4.5" /></div><div><p className="text-sm font-black">Isolated staging send</p><p className="mt-1 text-xs leading-5 text-[var(--muted)]">Production NexiMail is not touched. This panel is capped at {runtimePolicy.maxRecipientsPerCampaign?.toLocaleString() ?? "—"} active recipients per campaign.</p></div></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-[.1em] ${runtimePolicy.sendingEnabled ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-amber-500/10 text-amber-700 dark:text-amber-300"}`}>{runtimePolicy.sendingEnabled ? "Test sending enabled" : "Sending locked"}</span></div> : null}
    ...
    <div className="flex flex-wrap justify-end gap-2"><button disabled={busy} className="btn-secondary" type="submit">Save draft</button><button disabled={busy || (staging && !runtimePolicy.sendingEnabled)} className="btn-primary" type="button" onClick={(event)=>{const form=event.currentTarget.form;if(form) submit(new FormData(form),"queue");}}>{staging ? "Queue test campaign" : "Queue / schedule"}</button></div>
  </form>;
}
