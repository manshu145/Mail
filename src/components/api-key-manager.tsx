...
export function ApiKeyManager({ disabled }: { disabled?: boolean }) {
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-[90] grid place-items-center overflow-y-auto bg-black/50 p-4 backdrop-blur-sm">
      <section className="premium-panel max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto p-5 sm:p-6">
        <div className="mb-5 flex items-start justify-between gap-4"><div><p className="page-eyebrow">Developer access</p><h2 className="mt-1 text-2xl font-black">Create API key</h2></div><button className="icon-button grid" onClick={() => { setOpen(false); setSecret(""); setError(""); }}><X className="h-4 w-4" /></button></div>
