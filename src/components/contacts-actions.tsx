  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button disabled={!databaseConfigured || busy} onClick={() => fileRef.current?.click()} className="inline-flex items-center gap-2 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm font-bold text-zinc-700 shadow-sm transition hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200 dark:hover:bg-zinc-900"><FileUp className="h-4 w-4" /> Import CSV</button>
        <button disabled={!databaseConfigured} onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-zinc-950 px-4 py-2.5 text-sm font-bold text-white shadow-[0_8px_30px_rgba(24,24,27,.18)] transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200"><Plus className="h-4 w-4" /> Add contact</button>
      </div>
      ...
      {open ? (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-black/50 p-4 backdrop-blur-sm">
          <section className="w-full max-w-md rounded-3xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-800 dark:bg-zinc-950">
            <div className="mb-6 flex items-start justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-zinc-400">Audience</p><h2 className="mt-1 text-2xl font-black tracking-tight">Add contact</h2></div><button onClick={() => setOpen(false)} className="rounded-lg p-2 text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900"><X className="h-5 w-5" /></button></div>
            <form action={submitContact} className="space-y-4">
              ...
              <button disabled={busy} className="w-full rounded-xl bg-zinc-950 px-4 py-3 text-sm font-extrabold text-white disabled:opacity-50 dark:bg-white dark:text-zinc-950">{busy ? "Saving…" : "Save contact"}</button>
            </form>
          </section>
        </div>
      ) : null}
