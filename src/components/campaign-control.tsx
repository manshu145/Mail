...
export function CampaignControl({ id, status, failed = 0 }: { id: string; status: string; failed?: number }) {
  ...
  async function run(action: Action) {
    ...
    if (action === "retry_failed" && !confirm(`Retry ${failed.toLocaleString()} failed recipient${failed === 1 ? "" : "s"}?`)) return;
    setBusy(action);
    setError("");
  ...
  return <div className="flex flex-wrap items-center justify-end gap-2">
    ...
    {error ? <span className="basis-full text-right text-xs font-semibold text-rose-600">{error}</span> : null}
  </div>;
}
