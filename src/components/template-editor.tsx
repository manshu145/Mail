...
export function TemplateEditor({ template, attachments }: { template: { id: string; name: string; subject: string | null; htmlBody: string; textBody: string }; attachments: TemplateAttachmentView[] }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[1.04fr_.96fr]">
    ...
    <section className="premium-panel overflow-hidden xl:sticky xl:top-[90px] xl:self-start"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] p-4"><div><h2 className="font-black">Live preview</h2><p className="mt-1 text-xs text-[var(--muted)]">Sample personalization values are rendered here.</p></div><div className="flex rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-1 text-xs font-bold"><button type="button" onClick={()=>setPreviewMode("desktop")} className={`rounded-lg px-3 py-1.5 ${previewMode==="desktop"?"bg-[var(--surface)] shadow-sm":"text-[var(--muted)]"}`}>Desktop</button><button type="button" onClick={()=>setPreviewMode("mobile")} className={`rounded-lg px-3 py-1.5 ${previewMode==="mobile"?"bg-[var(--surface)] shadow-sm":"text-[var(--muted)]"}`}>Mobile</button></div></div><div className="bg-[#e9edf5] p-4 sm:p-6"><div className={`mx-auto overflow-hidden rounded-xl border border-zinc-300 bg-white shadow-sm transition-all ${previewMode==="mobile"?"max-w-[390px]":"max-w-[760px]"}`}><iframe title="Template preview" sandbox="" srcDoc={preview} className="h-[760px] w-full bg-white" /></div></div></section>
  </div>;
}
