...
import { Activity, BarChart3, ChevronDown, ContactRound, Gauge, Layers3, Mail, Menu, Send, ServerCog, Settings, ShieldBan, X } from "lucide-react";
      ...
      <div className="mt-auto border-t border-black/[0.06] p-3 dark:border-white/[0.07]">
        <div className="rounded-2xl border border-black/[0.05] bg-white/75 p-3 shadow-[0_1px_3px_rgba(0,0,0,.03)] dark:border-white/[0.07] dark:bg-white/[0.035]">
          <div className="flex items-center gap-3">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-gradient-to-br from-zinc-800 to-zinc-950 text-xs font-bold text-white ring-1 ring-black/10 dark:from-zinc-700 dark:to-zinc-900 dark:text-white">{session.name.slice(0, 2).toUpperCase()}</div>
          ...
          <form action="/api/auth/logout" method="post" className="mt-3"><button className="btn-secondary w-full !min-h-9 !py-2 text-xs" type="submit">Sign out</button></form>
        </div>
      </div>
    </div>
  );

  return <div className="min-h-dvh bg-[#f6f5f2] text-zinc-950 dark:bg-[#0d0d0e] dark:text-white">
    ...
    <div className="lg:pl-[268px]">
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-black/[0.055] bg-[#f6f5f2]/88 px-4 backdrop-blur-xl sm:px-6 lg:px-8 dark:border-white/[0.065] dark:bg-[#0d0d0e]/88">
        <div className="flex items-center gap-3"><button aria-label="Open navigation" className="btn-secondary !min-h-0 !p-2 lg:hidden" onClick={() => setOpen(true)}><Menu className="h-5 w-5" /></button><div className="lg:hidden"><BrandMark compact /></div><div className="hidden text-sm font-semibold text-zinc-500 sm:block">NexiMail control plane</div></div>
