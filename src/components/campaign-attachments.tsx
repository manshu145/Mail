...
export function CampaignAttachments({ campaignId }: { campaignId: string }) {
  ...
  async function upload(file: File) {
    ...
    const response = await fetch(`/api/campaigns/${campaignId}/attachments`, { method: "POST", body: form });
    const data = await response.json().catch(() => ({})) as { error?: string; attachment?: CampaignAttachmentView };
    setBusy(false);
  ...
  async function remove(id: string) {
    setBusy(true); setError("");
    const response = await fetch(`/api/campaigns/${campaignId}/attachments`, { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ attachmentId: id }) });
