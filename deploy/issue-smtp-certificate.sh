...
for tool in certbot nginx docker openssl curl; do command -v "$tool" >/dev/null || { echo "$tool is required"; exit 1; }; done
command -v dig >/dev/null || { echo "dig is required (install dnsutils/bind-tools)."; exit 1; }

echo "=== DNS / host preflight ==="
PUBLIC_IP=$(curl -4fsS --max-time 10 https://api.ipify.org)
mapfile -t CF_RESOLVED < <(dig @1.1.1.1 +short A "$HOST" | grep -E '^[0-9]+(\.[0-9]+){3}
...
echo "SMTP TLS setup complete for $HOST."
 | sort -u)
mapfile -t GG_RESOLVED < <(dig @8.8.8.8 +short A "$HOST" | grep -E '^[0-9]+(\.[0-9]+){3}
...
echo "SMTP TLS setup complete for $HOST."
 | sort -u)
printf 'Host: %s\nPublic IPv4: %s\nCloudflare A: %s\nGoogle A: %s\n' "$HOST" "$PUBLIC_IP" "${CF_RESOLVED[*]:-none}" "${GG_RESOLVED[*]:-none}"
MATCH=0
for ip in "${CF_RESOLVED[@]}" "${GG_RESOLVED[@]}"; do [[ "$ip" == "$PUBLIC_IP" ]] && MATCH=1; done
if [[ "$MATCH" -ne 1 ]]; then
  echo "[FAIL] Public DNS for $HOST does not resolve to this VPS ($PUBLIC_IP). Fix/propagate its A record before issuing the certificate."
  echo "Local /etc/hosts or getent results are intentionally ignored."
  exit 3
fi
