...
async function heartbeat(meta: Record<string, unknown> = {}) {
  await db
    ...
    .values({ workerName: "transport", metadata: meta })
    .onConflictDoUpdate({
...
async function accountWithinLimits(account: typeof sendingAccounts.$inferSelect) {
  const [warmup] = await db
    .select()
    .from(sendingAccountWarmups)
    .where(eq(sendingAccountWarmups.sendingAccountId, account.id))
    .limit(1);
  ...
  return {
    allowed: Number(row.hour_count || 0) < account.hourlyLimit && Number(row.day_count || 0) < effectiveDaily,
    effectiveDaily,
  };
}
...
async function claimMessages(): Promise<Claimed[]> {
  const result = await pool.query<Claimed>(
    `with picked as (
     ...
     returning m.id, m.attempt_count`,
    [claimBatch],
  );
  return result.rows;
}
...
async function markDeferred(id: string, attempt: number, error: unknown) {
  ...
  if (attempt >= maxAttempts) {
    await pool.query(
      `update messages set status='failed', last_error=$2, next_attempt_at=null where id=$1 and status='sending'`,
      [id, detail],
    );
  ...
  await pool.query(
    `update messages
     set status='deferred', last_error=$2, next_attempt_at=now() + ($3::int * interval '1 second')
     where id=$1 and status='sending'`,
    [id, detail, delaySeconds],
  );
...
async function releaseThrottled(id: string) {
  await pool.query(
    `update messages
     set status='ready_for_transport', next_attempt_at=now() + interval '60 seconds', last_error='sending_account_rate_limited'
     where id=$1 and status='sending'`,
    [id],
  );
}
...
async function recoverStaleClaims() {
  await pool.query(
    `update messages
     set status='failed', next_attempt_at=null, last_error='transport_state_uncertain_after_worker_restart'
     ...
     where c.status='sending'
       ...
       and not exists (
         ...
         where m.campaign_id=c.id
           and m.status in ('queued','ready_for_transport','sending','deferred')
       )`,
  );
}
...
async function runOnce() {
  ...
  for (const claim of claimed) {
    ...
    if (!campaign || !contact || !campaign.sendingAccountId || !campaign.templateId || campaign.status === "cancelled") {
      await pool.query(
        `update messages set status='failed', last_error='campaign_transport_configuration_incomplete', next_attempt_at=null where id=$1 and status='sending'`,
        [message.id],
      );
      failed++;
      continue;
    ...
    if (!account || account.status !== "active" || !template) {
      await pool.query(
        `update messages set status='failed', last_error='sending_account_or_template_unavailable', next_attempt_at=null where id=$1 and status='sending'`,
        [message.id],
      );
      failed++;
      continue;
    ...
    if (!limit.allowed) {
      await releaseThrottled(message.id);
      throttled++;
      continue;
    }
    ...
    const raw = [
      ...
      `Message-ID: <${message.id}@${fromEmail.split("@")[1] || "neximail.local"}>`,
      `X-NexiMail-Message-ID: ${message.id}`,
      "MIME-Version: 1.0",
    ...
    try {
      ...
      await pool.query(
        `update messages
         set status='mta_accepted', accepted_at=now(), provider_message_id=$2, last_error=null, next_attempt_at=null
         where id=$1 and status='sending'`,
        [message.id, result.queueId],
      );
      accepted++;
    } catch (error) {
...
async function main() {
  ...
  while (true) {
    try {
      ...
      console.error("[transport-worker]", error);
