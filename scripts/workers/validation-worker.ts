...
`));
  ...
  if (scope === "gmail:unresolved") {
    return db.select().from(contacts).where(and(eq(contacts.status, "active"), inArray(contacts.validationStatus, ["pending","unknown","error"]), sql`lower(${contacts.normalizedEmail}) ~ '@(gmail|googlemail)\\.com
}
...
`));
  ...
  // Legacy full-Gmail scopes are intentionally narrowed to unresolved contacts so
  // accepted/valid/invalid mailbox outcomes are never re-probed automatically.
  return db.select().from(contacts).where(and(eq(contacts.status, "active"), inArray(contacts.validationStatus, ["pending","unknown","error"]), sql`lower(${contacts.normalizedEmail}) ~ '@(gmail|googlemail)\\.com
}
