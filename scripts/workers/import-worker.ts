...
async function heartbeat(metadata: Record<string, unknown>) {
  await db
    ...
    .values({ workerName: "import", metadata })
    .onConflictDoUpdate({ target: workerHeartbeats.workerName, set: { lastSeenAt: new Date(), metadata } });
}

async function importListId(jobId: string) {
  const result = await pool.query<{ list_id: string | null }>("select list_id from import_jobs where id=$1", [jobId]);
  return result.rows[0]?.list_id || null;
}

async function attachToList(contactId: string, listId: string | null) {
  if (!listId) return;
  await db
    .insert(contactLists)
    .values({ contactId, listId })
    .onConflictDoNothing({ target: [contactLists.contactId, contactLists.listId] });
}
...
async function runOnce() {
  const [job] = await db
    .select()
    .from(importJobs)
    .where(sql`${importJobs.status} in ('pending','processing')`)
  ...
  const staged = await db
    ...
    .where(and(eq(importStagingRows.jobId, job.id), eq(importStagingRows.processed, false)))
    .orderBy(importStagingRows.rowNumber)
    .limit(batchSize);
  ...
  for (const row of staged) {
    ...
    if (!item?.email || !isValidEmail(item.email)) {
      await db
        ...
        .set({ processed: true, result: "invalid", detail: "invalid_email" })
    ...
    if (suppressed) {
      await db
        .update(importStagingRows)
        .set({ processed: true, result: "suppressed", detail: String(suppressed.reason) })
        .where(eq(importStagingRows.id, row.id));
      continue;
    }
    ...
    await db
      .update(importStagingRows)
      .set({ processed: true, result, detail: contactId ? null : "contact_lookup_failed" })
      .where(eq(importStagingRows.id, row.id));
  }

  const counts = await db.execute(sql`
    select
      count(*) filter(where result='imported')::int as imported,
      count(*) filter(where result='duplicate')::int as duplicates,
      count(*) filter(where result='invalid')::int as invalid,
      count(*) filter(where result='suppressed')::int as suppressed,
      count(*) filter(where processed=false)::int as remaining
  ...
  await db
    ...
    .set({
      ...
      invalidRows: invalid,
      errorMessage: suppressed ? `${suppressed} globally suppressed row(s) skipped` : null,
