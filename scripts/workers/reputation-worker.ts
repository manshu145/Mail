...
async function run(){
 ...
 const bounceStop=settings.reputationBounceStopRate, complaintStop=settings.reputationComplaintStopRate, minSample=settings.reputationMinSample;
 ...
 for(const account of accounts){
  ...
  const sent=Number(x.sent||0), bounced=Number(x.bounced||0), deferred=Number(x.deferred||0), complaints=Number(x.complaints||0);
  const bounceRate=sent?bounced/sent:0, complaintRate=sent?complaints/sent:0;
  const risk=sent>=minSample&&(bounceRate>=bounceStop||complaintRate>=complaintStop);
  if(risk)atRisk++;
