...
async function heartbeat(meta: Record<string, unknown> = {}) {
  await db
    ...
    .values({ workerName: "campaign", metadata: meta })
    .onConflictDoUpdate({
...
async function resolveRecipients(list: typeof lists.$inferSelect) {
  ...
  if (rule.field === "email_domain") {
    condition = rule.operator === "equals"
      ? ilike(contacts.normalizedEmail, `%@${rule.value}`)
      : ne(contacts.normalizedEmail, rule.value);
  } else if (rule.field === "validation_status") {
    condition = rule.operator === "equals"
      ? eq(contacts.validationStatus, rule.value as "pending" | "valid" | "invalid" | "unknown" | "error")
      : ne(contacts.validationStatus, rule.value as "pending" | "valid" | "invalid" | "unknown" | "error");
  } else {
    condition = rule.operator === "equals"
...
async function runOnce() {
  ...
  const due = await db
    .select()
    .from(campaigns)
    .where(or(eq(campaigns.status, "queued"), and(eq(campaigns.status, "scheduled"), lte(campaigns.scheduledAt, now))))
    .limit(10);
  ...
  for (const campaign of due) {
    if (!campaign.listId) {
      await blockCampaign(campaign.id, "campaign.worker_missing_list", {});
      blocked++;
      continue;
    }
    ...
    if (!list) {
      await blockCampaign(campaign.id, "campaign.worker_list_not_found", { listId: campaign.listId });
      blocked++;
      continue;
    }
    ...
    if (policy.maxRecipientsPerCampaign !== null && recipients.length > policy.maxRecipientsPerCampaign) {
      await blockCampaign(campaign.id, "campaign.staging_recipient_limit_blocked", {
        recipients: recipients.length,
        limit: policy.maxRecipientsPerCampaign,
      });
      blocked++;
      continue;
    }
    ...
    if (!recipients.length) {
      await blockCampaign(campaign.id, "campaign.worker_empty_audience", { listId: list.id });
      blocked++;
      continue;
    }
