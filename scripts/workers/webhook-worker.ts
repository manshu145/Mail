...
async function heartbeat(metadata: Record<string, unknown>) {
  await db
    ...
    .values({ workerName: "webhook", metadata })
    .onConflictDoUpdate({
...
async function recoverStale() {
  await pool.query(
    `update webhook_deliveries
     set status='pending', next_attempt_at=now(), updated_at=now(), response_text='recovered_stale_claim'
...
async function claim(): Promise<Claimed[]> {
  const result = await pool.query<Claimed>(
    `with picked as (
       ...
       set status='processing', attempt_count=wd.attempt_count+1, updated_at=now()
       from picked
     ...
     from claimed c
     join webhook_endpoints we on we.id=c.webhook_endpoint_id
...
async function deliver(item: Claimed) {
  if (!item.endpoint_active) {
    await pool.query(
      `update webhook_deliveries set status='failed', response_text='endpoint_disabled', next_attempt_at=null, updated_at=now() where id=$1`,
      [item.delivery_id],
    );
  ...
  const secret = decryptWebhookSecret(item.secret_ciphertext);
  ...
  try {
    ...
    if (response.ok) {
      await pool.query(
        `update webhook_deliveries
         set status='delivered', response_code=$2, response_text=$3, delivered_at=now(), next_attempt_at=null, updated_at=now()
         where id=$1 and status='processing'`,
        [item.delivery_id, response.status, responseText],
      );
    ...
    if (item.attempt_count >= maxAttempts) {
      await pool.query(
        `update webhook_deliveries
         set status='failed', response_code=$2, response_text=$3, next_attempt_at=null, updated_at=now()
         where id=$1 and status='processing'`,
        [item.delivery_id, response.status, responseText || `HTTP ${response.status}`],
      );
      return "failed" as const;
    }
    ...
    await pool.query(
      `update webhook_deliveries
       set status='pending', response_code=$2, response_text=$3,
           next_attempt_at=now()+($4::int * interval '1 second'), updated_at=now()
       where id=$1 and status='processing'`,
      [item.delivery_id, response.status, responseText || `HTTP ${response.status}`, delay],
    );
    return "retry" as const;
  } catch (error) {
    const message = error instanceof Error ? error.message.slice(0, 1000) : "webhook_request_failed";
    if (item.attempt_count >= maxAttempts) {
      await pool.query(
        `update webhook_deliveries
         set status='failed', response_text=$2, next_attempt_at=null, updated_at=now()
         where id=$1 and status='processing'`,
        [item.delivery_id, message],
      );
      return "failed" as const;
    }
    ...
    await pool.query(
      `update webhook_deliveries
       set status='pending', response_text=$2,
           next_attempt_at=now()+($3::int * interval '1 second'), updated_at=now()
       where id=$1 and status='processing'`,
      [item.delivery_id, message, delay],
    );
    return "retry" as const;
  }
}
...
async function main() {
  ...
  while (true) {
    try {
      ...
      console.error("[webhook-worker]", error);
